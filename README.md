...
* MSCN 논문 기반 구조를 코드 레벨에서 직접 재현
* SQL query를 tables / predicates / joins set으로 분해하여 모델 입력으로 변환

* 각 요소를 다음과 같이 featurization:
  - Table: one-hot encoding
  - Join: join graph 기반 binary encoding
  - Predicate:
    - column: one-hot encoding
    - operator (=, <, >): categorical encoding
    - value: min-max normalization

* variable-length set을 처리하기 위해 padding 및 mask 기반 처리 구현
* PyTorch 기반 학습 및 추론 파이프라인 구축

* 단순 one-hot 기반 표현은 column 간 correlation이나 data distribution을 충분히 반영하지 못하는 한계 존재
* 이를 해결하기 위해 predicate 간 correlation modeling을 목표로 attention 구조 설계
* 복잡한 join 조건에서 cardinality estimation 정확도 개선 가능성 확인
...
<p align="center">
  <img src="./ppt/architecture.png" width="400"/>
</p>
...
기존 MSCN은 set-wise average pooling을 사용하여
predicate 간 correlation이나 join dependency를 충분히 반영하지 못하는 한계가 있습니다.

query마다 predicate 간 상호작용이 다르다는 점에 착안하여,
이를 모델이 학습할 수 있도록 attention 구조를 도입했습니다.
