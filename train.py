...
def print_qerror(preds_unnorm, labels_unnorm):
    ...
    for i in range(len(preds_unnorm)):
        ...
        if isinstance(preds_unnorm[i], np.ndarray):
            pred = preds_unnorm[i].item()
        else:
            pred = preds_unnorm[i]

        if isinstance(labels_unnorm[i], np.ndarray):
            label = labels_unnorm[i].item()
        else:
...
def train_and_predict(workload_name, num_queries, num_epochs, batch_size, hid_units, cuda):
    ...
    # Write predictions
    file_name = "results/predictions_mscn" + workload_name + ".csv"
